...
public class GUI extends JFrame implements ActionListener {
	...
	private CourseCollection wC = new CourseCollection();
	...
	public void initialize() {
		...
		btnGetWordDocument = new JButton("Save Info to Word DOC");
		btnGetWordDocument.addActionListener(this);
		btnGetWordDocument.setBounds(118, 264, 183, 60);
		...
		btnGetClassMaterials.setBounds(225, 191, 177, 43);
		courseRegistration.add(btnGetClassMaterials);
		
		btnWishList = new JButton("Wish List");
		btnWishList.addActionListener(this);
		btnWishList.setBounds(21, 192, 183, 42);
		courseRegistration.add(btnWishList);
	...
	public void actionPerformed(ActionEvent e) {
		...
		// action for button to enroll new student
			...
			clearConsole();
			if (isNewStudentInfoGood()) {
				s1 = new Student(fullNameTXTField.getText(), birthDateTXTField.getText(), newPasswordTXTField.getText(),
						null, null);
				...
				outputArea.append("Invaild Inputs!\nPlease Make sure all boxes are Correctly Filled out.\nThanks!\n");
			}
		}
		...
		// action for button to add a course to a student
			...
			if (course.getStudentCount() < 20) {
				...
				s1.wishList.add(course);
				outputArea.append("This Class is full it has been added to your wish List\n");
			}
		}
		...
		else if (nameOfCallingBtn.equals("Save Info to Word DOC")) {
		...
		} else if (nameOfCallingBtn.equals("Wish List")) {
			outputArea.append(s1.wishList.wishListToString());
			btnWishList.setEnabled(false);
		}
	}

	// void method to show and decide what the next options are after registering
	// for a course
	public void courseRegOptions() {
		String[] options = { "Yes", "Quit" };
		int optionsPane = JOptionPane.showOptionDialog(null, "Would You like to Register for More Courses?",
				"Thank You!", JOptionPane.DEFAULT_OPTION, JOptionPane.INFORMATION_MESSAGE, null, options, options[0]);
		if (optionsPane == 0) {
	...
	// void method to display and decide what next options are after enrolling new
	// student
	public void newEnrollOptions() {
		String[] options = { "Yes", "Quit" };
		int optionsPane = JOptionPane.showOptionDialog(null, "Would You like to Register for Courses?", "Thank You!",
	...
	/**
	 * checks text fields when new student is created
	 * 
	 * @return boolean, based on formating of input
	 */
	public boolean isNewStudentInfoGood() {
		String[] testTwoNames = fullNameTXTField.getText().split(" ");
		boolean isName = false;
		boolean isBirthdate = false;
		boolean isPassword = false;
		if (testTwoNames[0] != null && testTwoNames[1] != null)
			isName = true;
		if (birthDateTXTField.getText().charAt(2) == '/' && birthDateTXTField.getText().charAt(5) == '/')
			isBirthdate = true;
		if (passwordTXTField.getText() != null)
			isPassword = true;
		if (isName && isBirthdate && isPassword) {
			return true;
		} else {
			return false;
		}
